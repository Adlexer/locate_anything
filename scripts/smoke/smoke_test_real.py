...
print(
    f"[smoke] model loaded in {time.time()-t0:.1f}s; attn={worker.model.language_model.model._attn_implementation}",
    flush=True,
)
...
t2 = time.time()
res2 = worker.ground_multi(
    img, "people", generation_mode="hybrid", max_new_tokens=2048, verbose=False
)
...
t3 = time.time()
res3 = worker.point(
    img, "the bus", generation_mode="hybrid", max_new_tokens=512, verbose=False
)
pts = []
...
print(
    f"[smoke] times: total={time.time()-t0:.1f}s detect={t2-t1:.1f}s ground={t3-t2:.1f}s",
    flush=True,
)
print(
    "[smoke] outputs:",
    f"{OUT_DIR}/smoke_real_annotated.jpg",
    f"{OUT_DIR}/smoke_real_results.json",
    flush=True,
)
