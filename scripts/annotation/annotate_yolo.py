...
def parse_answer(answer):
    ...
    for m in BOX_RE.finditer(answer):
        if m.group(1) is not None:
            ...
            items.append((cur_label, int(m.group(2)), int(m.group(3)),
...
def sample_video_frames(video_path, out_dir, n_frames, step):
    ...
    if step and step > 0:
        ...
        idxs = [int(round(i * (total - 1) / max(1, n_frames - 1))) for i in range(min(n_frames, total))]
...
def main():
    ap = argparse.ArgumentParser(description="LocateAnything zero-shot -> YOLO annotation")
    ap.add_argument("--data", required=True, help="dataset root (walked recursively)")
    ap.add_argument("--classes", required=True, help='comma-separated class names, e.g. "gas cylinder,electric scooter,bicycle"')
    ap.add_argument("--model", default="/home/xu/models/LocateAnything-3B")
    ap.add_argument("--out", default=None, help="aux output dir (summary/previews/manifest); default <repo>/outputs/annotation_detect")
    ap.add_argument("--video-frames", type=int, default=8, help="max sampled frames per video")
    ap.add_argument("--video-step", type=int, default=0, help="sample every N-th frame (0 = evenly spread)")
    ...
    ap.add_argument("--overwrite", action="store_true", help="re-annotate even if txt exists")
    ...
    for dirpath, dirnames, filenames in os.walk(data):
        ...
        for f in filenames:
            ...
            if ext in IMAGE_EXTS:
                ...
                videos.append(p)
    images.sort(); videos.sort()
    print(f"[annotate] classes={list(classes)} images={len(images)} videos={len(videos)}")
    ...
    summary = {"classes": list(classes), "images": {}, "videos": {},
               "total_boxes": 0, "class_counts": {c: 0 for c in classes}}
    manifest = []
    errors = []
    ...
    def annotate_image(img_path, manifest_rel):
        ...
        r = worker.detect(img, cats, generation_mode=args.mode,
                          ...
                          repetition_penalty=args.repetition_penalty, verbose=False)
        ...
        if not args.no_preview:
            draw_preview(img.copy(), items, classes,
    ...
    for p in images:
        ...
        try:
            ...
            summary["images"][rel] = {"boxes": 0 if n in ("skip", None) else n,
                                      "time_s": round(time.time() - t1, 2), "status": "skip" if n == "skip" else "ok"}
    ...
    for vp in videos:
        ...
        try:
            frames = sample_video_frames(vp, frames_dir, args.video_frames, args.video_step)
            ...
            for fp in frames:
                ...
                if n not in ("skip", None):
                    summary["total_boxes"] += n
                    summary["videos"][rel]["frames_with_boxes"] = summary["videos"][rel].get("frames_with_boxes", 0) + (1 if n else 0)
        except Exception as e:
    ...
    print(f"[annotate] done in {time.time()-t0:.1f}s total_boxes={summary['total_boxes']}")
