...
def parse_answer(answer):
    ...
    for m in BOX_RE.finditer(answer):
        if m.group(1) is not None:
            ...
            items.append({"class": cur, "x1": int(m.group(2)), "y1": int(m.group(3)),
...
def main():
    ...
    ap.add_argument("--root", required=True, help="dataset root for resolving image paths")
    ...
    for rel in imgs:
        ...
        r = worker.detect(img, classes, generation_mode=args.mode,
                          max_new_tokens=args.max_new_tokens, temperature=0.0,
                          top_p=0.9, top_k=0, repetition_penalty=1.1, verbose=False)
        results[rel] = {"raw": r["answer"], "boxes": parse_answer(r["answer"]),
                        "time_s": round(time.time() - t0, 2)}
        print(f"  {rel}: {len(results[rel]['boxes'])} boxes")

    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({"model": args.model, "classes": classes, "results": results},
