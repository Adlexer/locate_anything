...
Running（阶段四·闭环第二轮完成：数据扩充 2K（1998 图/3231 教师框）+ 调优训练 tw_run_v3 在人工 GT val mAP50 0.995（v1 0.757 / v2 0.504）——数据量瓶颈解决；**下一步=v3 抽样复核→v4、扩大 val、设备导出**）
...
- [闭环第二轮] 数据扩充+调优（报告 14）：采样 1998 图（wave2_ebike 1000 + ebike_like 998，剔除 battery/防泄漏）；教师标注 3231 框；组装 yolo_detect_tw2k（train 2106 / val 12 人工GT）；tw_run_v3（batch32/lr0.005/freeze10）best@49，人工 GT val mAP50 0.995 / mAP50-95 0.954（P0.924/R0.979），显著超越 v1(0.757)/v2(0.504)
...
- [规划] v3 抽样人工复核（工作表复用）→ 修正后增量重训 v4（只改必要框）
- [规划] 扩大独立测试集（当前 val 9 实例偏小）：从 2K 中再抽一批人工 GT
- [规划] 管线改进：annotate_yolo.py 加同目标跨类冲突标记；cross_check.py 加同类碎片合并
- [规划] 目标设备导出（Jetson TRT FP16/INT8）+ 精度回归；双卡 DDP 进阶
...
- [2026-08-16 19:0x] 用户同意下一步编排：数据扩到 2K、训练调优，暂不做全量审核/全量训练
- [2026-08-16 19:1x] 采样 1998 图（exclude 已采样 150、跳过 2 损坏）；教师 2 类标注 3231 框（31 分钟）
- [2026-08-16 19:2x] 组装 yolo_detect_tw2k（train 2106 / val 12 人工GT）；tw_run_v3 调优训练（batch32/lr0.005/freeze10，best@49）
- [2026-08-16 19:3x] 公平对比：v3 mAP50 0.995 / v1 0.757 / v2 0.504 → 数据量瓶颈解决；报告 14 落盘
