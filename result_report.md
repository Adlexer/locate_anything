...
- [2026-08-16] 两轮车闭环第二轮里程碑（报告 14）：数据扩充至 1998 图（教师伪标签 3231 框）+ 调优训练 tw_run_v3（batch32/lr0.005/freeze10），在人工 GT val（12 图/9 实例）上 mAP50 0.995 / mAP50-95 0.954（P 0.924 / R 0.979），显著超越 v1（0.757）与 v2（0.504）——确认"数据量是首要瓶颈"，扩数据+调优后 YOLO 追平/超越教师；新增 2K 采样/标注/组装/调优/对比脚本
